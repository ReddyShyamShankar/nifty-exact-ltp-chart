...
Historical fills do not come from the current-day trades endpoint. Import the Zerodha tradebook CSV once as the historical baseline. The extension stores later current-day trades as immutable evidence and deduplicates repeated refreshes. For every new trade, explicitly select its same-expiry strategy owner and press **ASSIGN REVIEWED TRADES**. Confirmed contiguous daily evidence extends the baseline without another CSV; unassigned or gapped evidence keeps publication withheld. Manually allocate whole signed lots, then explicitly accept the snapshot before chart publication. **HISTORY GAP** evidence withholds the affected whole-trade map.
