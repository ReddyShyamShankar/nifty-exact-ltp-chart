...
Hardened Task 4 axis calibration capture after commit `8668690`. Scope remains `extension-axis-ladder/background.js`, its focused capture contract test, and this report.
...
- Exit 1 with one adversarial failure: a capture borrowed a trusted debugger, `TRUSTED_SESSION_END` ran during capture, and cleanup leaked the debugger because `attachedHere` was false.
...
Result: exit 0; 14/14 focused capture contract tests passed.
...
- Made debugger release lease-aware: capture cleanup removes its capture lease before detaching, then detaches whenever no trusted lease, active capture, or queued capture lease remains. A trusted session still prevents detachment.
- Added an adversarial contract test covering a borrowed capture followed by `TRUSTED_SESSION_END` during capture. Concurrent captures now share one debugger lease and detach after final capture cleanup.
...
- Added short-lived debugger ownership: Accessibility is enabled for the capture, DOM is enabled for DOM-tree/box lookup, and the physical debugger session is detached only after all trusted and capture leases release.
